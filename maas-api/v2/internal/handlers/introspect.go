...
func (h *IdentityHandler) Introspect(c *gin.Context) {
	...
	if err != nil {
		log.Printf("DEBUG: Invalid team UUID: %v", err)
		c.JSON(http.StatusUnauthorized, IntrospectResponse{
			Active: false,
			Error:  "invalid_key",
		})
	...
	log.Printf("DEBUG: Found team: %s with rate limits: %d/%s", team.Name, team.RateLimit, team.RateWindow)
...
func (h *IdentityHandler) verifyAPIKey(providedKey, storedHash, salt string) bool {
	...
	// Direct key comparison (like the old system)
	// The stored "hash" is actually the plaintext key for now
	log.Printf("DEBUG: Using direct key comparison")
