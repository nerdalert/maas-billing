...
CREATE TABLE teams (
  ...
  description  TEXT,
  default_policy_id UUID,                      -- FK -> policies (nullable)
  created_at   TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at   TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- 5) model_grants (model-level RBAC; team-wide or per-user)
CREATE TABLE model_grants (
  ...
  UNIQUE (team_id, COALESCE(user_id, '00000000-0000-0000-0000-000000000000'), model_id)
);

-- 6) policies (store full Kuadrant RLP/TRL spec verbatim in JSONB)
...
CREATE TABLE usage_metrics (
  ...
  UNIQUE (window_start, window_end,
          ...
          COALESCE(model_id,'00000000-0000-0000-0000-000000000000'))
);
...
CREATE INDEX idx_model_grants_team_user ON model_grants(team_id, user_id);
